...
int main() {
    ...
    for (ri i = 1; i <= n; i++) {    //对每一个a[i]都把f[1~i-1]遍历一遍才得出
        ...
        f[i]++;//这个1为a[i]自身的长度，因为f[i]初始为0
        ans = max(ans, f[i]);//ans不能省，不能输出f[n]，因为如果a[n]很小的话，f[n]=1
    }
    printf("%d\n", ans);
}
...
我们利用结构体将a数组的值和编号捆在一起，按值大小将数组排序 
...
struct su {
    ...
    inline bool operator<(const su b) {//将值从大到小排序，用于遍历
        if (v == b.v) return id > b.id;//按照序号从大到小可以保证所求为上升子序列
                                        //求严格上升的时候 大的id在前面；允许<=的时候，小的id在前
        return v < b.v;
    }
} a[maxn];
...
每一次对 F[] 数组二分查找，找到 小于A[i]的最大的F[j] ,并用它将 F[j+1]更新。
...
int main() {
    ...
    for (ri i = 1; i <= n; i++) scanf("%d", &a[i]);
    ans = 1;
    f[1] = a[1];//目前最大序列长度为1，f[1]为a[1]
    for (ri i = 2; i <= n; i++) {
        l = 1;
        r = ans;//每次遍历r取ans,即为当前最大长度
        //a[i]在f[l,r]的左边时，更新f[l]，在右边时，建立f[r+1]，在中间时找到比a[i]大的最小f[l]并更新f[l]
        while (l <= r) {//刚开始l==r，不能退出
            mid = (l + r) >> 1;//位运算快于乘积
            if (a[i] <= f[mid]) r = mid - 1;//要保证循环退出时l能取得满足a[i]<f[l]的最大值，等于都不行
            else l = mid + 1;
        }
        f[l] = a[i];//更新长度为l的末尾最小元素或者创建新长度的序列
        if (l > ans) ans++;//以l为结尾的序列，l>ans，说明遍历到了f的右边界，没有比a[i]大的f
    }
    printf("%d\n", ans);
}
