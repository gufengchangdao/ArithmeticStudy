...
double path_len(int * arr)
{
    ...
    for(int i=0;i<N-1;i++){	 
        int index1 = *(arr+i);	//当前城市	
        int index2 = *(arr+i+1);	//下一座城市 
        double dis = distance(city_pos[index1-1],city_pos[index2-1]);
        path += dis;
    }
    int last_index = *(arr+N-1); // 最后一个城市序号
    int first_index = *arr; // 第一个城市序号
    double last_dis = distance(city_pos[last_index-1],city_pos[first_index-1]);//回到出发点 
    path +=last_dis;
...
void create_new()
{
    int pos1 = (int)(N*((double)rand())/(RAND_MAX+1.0)); //第一个交叉点的位置，找到随机的两个城市 
    int pos2 = (int)(N*((double)rand())/(RAND_MAX+1.0));//除以RAND_MAXN+1是为了获得[0.0,1.0)的随机数 
...
int main(void)
{
    ...
    while(T > T_end){ // 当温度低于结束温度时，退火结束
        for(int i=0;i<L;i++){	//每个温度要迭代L次才降温一次 
            memcpy(city_list_copy,city_list,N*sizeof(int)); // 复制数组
            create_new(); // 产生新解
            f1 = path_len(city_list_copy);	//旧的 
            f2 = path_len(city_list);	//新的 
            df = f2 - f1;
            // 以下是Metropolis准则
            if(df >= 0){
                if(exp(-df/T) <= ((double)rand())/(RAND_MAX)) // 保留原来的解
                    memcpy(city_list,city_list_copy,N*sizeof(int));//顺序跟别人的有点不一样 
            }
        }
        T *= q; // 降温
        count++;
